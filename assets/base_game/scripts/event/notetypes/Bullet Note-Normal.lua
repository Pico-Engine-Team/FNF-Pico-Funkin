function onCreate()
	for i = 0, getProperty('unspawnNotes.length') - 1 do
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'Bullet Note-Normal-Normal-Normal-Normal' then
			setPropertyFromGroup('unspawnNotes', i, 'texture', 'scripts/images/notetypes/BULLET')
...
function goodNoteHit(id, direction, noteType, isSustainNote)
	if noteType == 'Bullet Note-Normal' then
		playSound('engine/shoot', 0.7)

		characterPlayAnim('boyfriend', 'attack', true);
		characterPlayAnim('Opponent', 'dodge', true);
