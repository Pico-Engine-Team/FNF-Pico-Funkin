...
// Pico Enigne
import lucas.states.funkin.EnginePreload;
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		// Pico Engine stages
		EnginePreload.addstage(curStage);
		...
		botplayTxt = new FlxText(400, healthBar.y - 90, FlxG.width - 800, Language.getPhrase("PicoBot").toUpperCase(), 32);
		...
		for (notetype in noteTypes)
			startLuasNamed('scripts/event/notetypes/' + notetype + '.lua');
		for (event in eventsPushed)
			startLuasNamed('scripts/event/' + event + '.lua');
		#end

		#if HSCRIPT_ALLOWED
		for (notetype in noteTypes)
			startHScriptsNamed('scripts/event/notetypes/' + notetype + '.hx');
		for (event in eventsPushed)
			startHScriptsNamed('scripts/event/' + event + '.hx');
		#end
		noteTypes = null;
		eventsPushed = null;
	...
	function set_health(value:Float):Float // You can alter how icon animations work here
	{
		...
		if(iconP1.animation != null && iconP1.animation.curAnim != null) iconP1.animation.curAnim.curFrame = (healthBar.percent < 20) ? 1 : 0; //If health is under 20%, change player icon to frame 1 (losing icon), otherwise, frame 0 (normal)
		if(iconP2.animation != null && iconP2.animation.curAnim != null) iconP2.animation.curAnim.curFrame = (healthBar.percent > 80) ? 1 : 0; //If health is over 80%, change opponent icon to frame 1 (losing icon), otherwise, frame 0 (normal)
		return health;
	}
